...
export default function WholesalePOSPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeProducts = onSnapshot(productsCollection, (snapshot) => {
      ...
      setProducts(productsData.filter(p => p.wholesalePrice && p.wholesalePrice > 0)); // Only show products with wholesale price
  ...
  const addToCart = (product: Product) => {
    setCart((prevCart) => {
      ...
      return [...prevCart, { ...product, quantity: 1, useWholesale: true }]; // Default to wholesale
    });
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-start h-full">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 h-full order-last md:order-first">
          <div className="flex-1 flex flex-col gap-6 min-h-0">
            <h1 className="font-headline text-3xl font-bold tracking-tight">Wholesale POS</h1>
            <Card>
                    ...
                    {filteredProducts.map((product) => (
                      ...
                      >
                        ...
                        <div className="p-3 bg-card">
                          ...
                          <p className="text-sm text-muted-foreground">KSH {(product.wholesalePrice || 0).toFixed(2)}</p>
                        </div>
                      </Card>
                    ))}
                  </div>
